...
RobotPose getRobotPose (int robotID)  // pulls the pose of the given robot # passed in by robotID-----------------------------
{
  ...
  //do{
    ...
    taskEXIT_CRITICAL(&robotPoseMutex);
    //D_println("looking for Robert");
  //} while(retval.ID == 0);
  //printRobotPose(retval);
  return retval;
}

int getBallPositions (BallPosition (&pos)[NUM_BALLS]) // returns number of balls and updates the passed vector with of all visible ball locations------------------------
{  
  int retval = 0;
  ...
  //D_print("getBallPositions Called: "); printBallPositions(numBalls, ballPositions);
  return retval;
}
